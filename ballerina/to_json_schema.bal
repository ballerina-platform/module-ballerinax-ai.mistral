...
type JsonSchemaType record {|
    string 'type?;
    (JsonSchemaType|JsonArraySchema)[] oneOf?;
    map<JsonSchemaType|JsonArraySchema|map<json>> properties?;
    string[] required?;
|};
...
isolated function generateJsonSchemaForTypedesc(typedesc<json> expectedResponseTypedesc, boolean nilableType) 
        returns JsonSchemaType|JsonArraySchema|map<json>|ai:Error {
...
isolated function getArrayMemberType(typedesc<json> expectedResponseTypedesc) returns typedesc<json> = @java:Method {
    ...
    'class: "io.ballerina.lib.ai.mistral.Native"
} external;
